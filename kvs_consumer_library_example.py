...
class KvsPythonConsumerExample:
    ...
    def service_loop(self):
        ...
        log.info(
            f'Getting KVS GetMedia Endpoint for stream: {KVS_STREAM01_NAME} ........')
        get_media_endpoint = self._get_data_endpoint(KVS_STREAM01_NAME,
                                                     'GET_MEDIA')

        # Get the KVS Media client for the GetMedia API call
        log.info(
            f'Initializing KVS Media client for stream: {KVS_STREAM01_NAME}........')
        kvs_media_client = self.session.client('kinesis-video-media',
                                               endpoint_url=get_media_endpoint)

        # Make a KVS GetMedia API call with the desired KVS stream and StartSelector type and time bounding.
        log.info(
            f'Requesting KVS GetMedia Response for stream: {KVS_STREAM01_NAME}........')
        ...
        log.info(
            f'Starting KvsConsumerLibrary for stream: {KVS_STREAM01_NAME}........')
        ...
        while True:
            ...
            log.info(
                "Nothn to see, just doin main application stuff in a loop here!")
            time.sleep(5)
    ...
    def on_fragment_arrived(self, stream_name, fragment_bytes, fragment_dom,
        ...
        try:
            ...
            log.info('')
            log.info(
                f'####### Fragment Receive and Processing Duration: {fragment_receive_duration} Secs')
            ...
            time_now = time.time()
            kvs_ms_behind_live = float(self.last_good_fragment_tags[
                                           'AWS_KINESISVIDEO_MILLIS_BEHIND_NOW'])
            producer_timestamp = float(self.last_good_fragment_tags[
                                           'AWS_KINESISVIDEO_PRODUCER_TIMESTAMP'])
            server_timestamp = float(self.last_good_fragment_tags[
            ...
            pretty_frag_dom = self.kvs_fragment_processor.get_fragement_dom_pretty_string(
                fragment_dom)
            ...
            ###########################################
            save_dir = 'fragments'
            frag_file_name = self.last_good_fragment_tags[
                                 'AWS_KINESISVIDEO_FRAGMENT_NUMBER'] + '.mkv'  # Update as needed
            ...
            log.info('')
            log.info(
                f'#######  Reading 1 in {one_in_frames_ratio} Frames from fragment as ndarray:')
            ndarray_frames = self.kvs_fragment_processor.get_frames_as_ndarray(
                fragment_bytes, one_in_frames_ratio)
                ...
                for label in response['Labels']:
                    if label['Name'] in ['Person']:
                        ret, buffer = cv2.imencode('.jpg',
                                                   cv2.cvtColor(ndarray_frame,
                                                                cv2.COLOR_BGR2RGB))
                        bytes_io = BytesIO(buffer)

                        s3_client.upload_fileobj(bytes_io,
                                                 'intruder-detection-bucket',
                                                 'detections/{}_{}.jpg'.format(
                                                     self.last_good_fragment_tags[
                                                         'AWS_KINESISVIDEO_FRAGMENT_NUMBER'],
                                                     str(i * one_in_frames_ratio)))

                        item_data = {
                            'fragment_number': self.last_good_fragment_tags[
                                'AWS_KINESISVIDEO_FRAGMENT_NUMBER'],
            ...
            jpg_file_base_name = self.last_good_fragment_tags[
                'AWS_KINESISVIDEO_FRAGMENT_NUMBER']
    ...
    def on_stream_read_complete(self, stream_name):
        ...
        print(
            f'Read Media on stream: {stream_name} Completed successfully - Last Fragment Tags: {self.last_good_fragment_tags}')
